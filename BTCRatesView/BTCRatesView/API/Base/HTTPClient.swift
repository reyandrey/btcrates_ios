...
class HTTPClient {
    ...
    func request(to endpoint: APIEndpoint, data: Data? = nil, completionHandler: @escaping (Result<Data?, RequestError>) -> Void) {
        httpManager.sendRequest(endpoint.method, to: endpoint.url, withPayload: data, headers: httpHeaders) { (data, response, error) in
            ...
            guard let response = response as? HTTPURLResponse else { completionHandler(.failure(.other(message: "Missing response"))); return }
